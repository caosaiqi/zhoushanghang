...
const Chat = {
	...
	mutations: {
		...
		updateMsgList(state, payload){
			const { chatType, chatId, msg, bySelf, type, id,key } = payload;
			
			keys=key
			console.log(keys,908872)
	...
	actions: {
		onGetContactUserList: function(context, payload,state){
			try{
				WebIM.conn.getRoster({
					success: function(roster){
						// console.log("roster0909090", roster,context, payload);
						// const { key } = payload;
						
						const userList = roster.filter(user => ["both", "to"].includes(user.subscription));
				// console.log(keys,89898)
		...
		onGetCurrentChatObjMsg: function(context, payload){
			console.log(context,12313123123)
